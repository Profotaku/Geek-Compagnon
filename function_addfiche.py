...
def ajouter_fiche_media(session):
    ...
    if titre_input in session.query(Succes.titre).all():
        return make_response(jsonify({'message': "Le titre du succès est déjà utilisé, et nous souhaitons garder une certaine originalité dans les succès!" }), 400)
    ...
    id_media = (session.query(func.max(Projets_Medias.id_medias)).scalar() or 0) + 1
    if 'dropz-visual' in request.files:
        url_image = request.files['dropz-visual']
        file_signature = url_image.read(8)
        #check signature of file detect if it's png or jpg
        ...
        if not os.path.exists(
                os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static/images/medias/', str(id_media))):
            os.mkdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static/images/medias/', str(id_media)))
        # save image in the relative folder
        url_image.seek(0)
        url_image.save(os.path.join(f'static/images/medias/{str(id_media)}/', "visual.jpg"))
        # open image for optimization
        saved_image = Image.open(f'static/images/medias/{str(id_media)}/visual.jpg')
        # save in same path with optimization
        wpercent = (460 / float(saved_image.size[0]))
        hsize = int((float(saved_image.size[1]) * float(wpercent)))
        saved_image.thumbnail((460, hsize), Image.LANCZOS)
        saved_image.save(f'static/images/medias/{str(id_media)}/visual.jpg', optimize=True, quality=95, progressive=True)
        relative_path = os.path.join('static/images/medias/', str(id_media), "visual.jpg")

    if 'dropz-success' in request.files:
        ...
        # save image in the relative folder
        url_image.seek(0)
        url_image.save(os.path.join(f'static/images/medias/{str(id_media)}/', "success.jpg"))
        # open image for optimization
        saved_image = Image.open(f'static/images/medias/{str(id_media)}/success.jpg')
        # save in same path with optimization
        wpercent = (460 / float(saved_image.size[0]))
        hsize = int((float(saved_image.size[1]) * float(wpercent)))
        saved_image.thumbnail((460, hsize), Image.LANCZOS)
        saved_image.save(f'static/images/medias/{str(id_media)}/success.jpg', optimize=True, quality=95, progressive=True)
        relative_path = os.path.join('static/images/medias/', str(id_media), "success.jpg")
    ...
    try:
        ...
        relative_path_fiche = os.path.join('static/images/medias/', str(id_media), "visual.jpg") if os.path.isfile('static/images/medias/'+ str(id_media)+"visual.jpg") else None
        # create table Fiche
        ...
        relative_path_succes = os.path.join('static/images/medias/', str(id_media), "success.jpg") if os.path.isfile('static/images/medias/'+ str(id_media)+"success.jpg") else None
        # create table Succes
        ...
        projet_media = Projets_Medias(id_projets_medias=(session.query(func.max(Projets_Medias.id_projets_medias)).scalar() or 0) + 1, id_fiches=fiche.id_fiches, id_notes=notes.id_notes, id_avis=avis.id_avis, id_succes=succes.id_succes)
            ...
            if alternative_name != "":
                nommer_m = Nommer_M(id_projets_medias=projet_media.id_projets_medias,
                                    nom_alternatif=session.query(Noms_Alternatifs).filter(
                                        Noms_Alternatifs.nom_alternatif == alternative_name).first().id_noms_alternatifs)
