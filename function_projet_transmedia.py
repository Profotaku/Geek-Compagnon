...
def projet_transmedia_app(session, id_projet_transmedia, client, user_agent):
	...
	if isadulte or transmedia_is_adulte or session.get('adulte', False):
		if client == 'app':
			...
			return render_template("public/projet_transmedia.html", activate_adulte_js_verification=False, **reponse)
	else:
		if client == 'app':
			return jsonify(reponse)
		else:
			return render_template("public/projet_transmedia.html", activate_adulte_js_verification=True, **reponse)
