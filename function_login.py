...
def login_app(bcrypt, session, auth_manager):
    pseudo = request.form['pseudo']
    password = request.form['password']
    user = session.query(Utilisateurs).filter_by(pseudo=pseudo).first()
    if not user or not bcrypt.check_password_hash(user.hash_mdp, password):
        return jsonify({"success": False, "errormessage": "Vos informations de connexion sont incorrectes. Veuillez réessayer."})
    elif not user.verifie:
        return jsonify({"success": False, "errormessage": "Ce compte n'a pas encore été activé. Veuillez cliquer sur le lien de validation dans le mail de confirmation d'inscription afin de pouvoir profiter de nos services."})
    elif user.desactive:
        return jsonify({"success": False, "errormessage": "Ce compte a été désactivé suite à une demande de l'utilisateur ou pour non respect de nos condition générales d'utilisation. Veuillez contacter l'administrateur du site pour plus d'informations."})
    ...
    return jsonify({"success": True, "auth_token": auth_token.signed, "refresh_token": refresh_token.signed})
