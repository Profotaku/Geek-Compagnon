...
def get_objective_data(id_projet_media, session):
	# Sous-requête pour obtenir l'id_fiche correspondant à l'id_produit_culturel
	subquery_fiche = session.query(
		Projets_Medias.id_fiches
	).filter(
		Projets_Medias.id_projets_medias == id_projet_media
	).subquery()
	...
	subquery_possession = session.query(
		Projets_Medias.id_projets_medias,
		func.coalesce(func.count(Posseder_M.pseudo), 0).label("nombre_possessions")
	).join(
		Posseder_M, Projets_Medias.id_projets_medias == Posseder_M.id_projets_medias, isouter=True
	).group_by(Projets_Medias.id_projets_medias).subquery()

	subquery_total = session.query(
		Projets_Medias.id_projets_medias,
		func.coalesce(func.count(Etre_Compose.id_produits_culturels), 0).label("produits_culturels_total")
	).join(
		Etre_Compose, Projets_Medias.id_projets_medias == Etre_Compose.id_projets_medias, isouter=True
	).group_by(Projets_Medias.id_projets_medias).subquery()

	subquery_commentaires = session.query(
		Projets_Medias.id_projets_medias,
		func.count(Etre_Commente_M.id_commentaires).label("nombre_commentaires")
	).select_from(Projets_Medias).outerjoin(
		Etre_Commente_M,
		Projets_Medias.id_projets_medias == Etre_Commente_M.id_projets_medias
	).group_by(Projets_Medias.id_projets_medias).subquery()
	...
	subquery_rank_note = session.query(
		...
		subquery_notes, Projets_Medias.id_fiches == subquery_notes.c.id_fiches
	).subquery()
	...
	subquery_rank_favoris = session.query(
		...
		subquery_favoris, Projets_Medias.id_fiches == subquery_favoris.c.id_fiches
	).subquery()
	...
	subquery_rank_possession = session.query(
		...
		subquery_possession, Projets_Medias.id_projets_medias == subquery_possession.c.id_projets_medias
	).subquery()
	...
	produit = session.query(
		Projets_Medias.nom_types_media,
		Fiches.nom,
		Fiches.synopsis,
		Fiches.contributeur,
		Fiches.url_image,
		...
		func.coalesce(subquery_total.c.produits_culturels_total, 0).label("produits_culturels_total"),
		...
		*[sub.c[f"proportion_note_{i}"] for i, sub in enumerate(subquery_repartition_notes)],
		...
		subquery_notes,
		Projets_Medias.id_fiches == subquery_notes.c.id_fiches
	).outerjoin(
		subquery_favoris,
		Projets_Medias.id_fiches == subquery_favoris.c.id_fiches
	).outerjoin(
		subquery_possession,
		Projets_Medias.id_projets_medias == subquery_possession.c.id_projets_medias
	).outerjoin(
		subquery_total,
		Projets_Medias.id_projets_medias == subquery_total.c.id_projets_medias
	).outerjoin(
		subquery_commentaires,
		Projets_Medias.id_projets_medias == subquery_commentaires.c.id_projets_medias
	).outerjoin(
		subquery_rank_note,
		Projets_Medias.id_projets_medias == subquery_rank_note.c.id_projets_medias
	).outerjoin(
		subquery_rank_favoris,
		Projets_Medias.id_projets_medias == subquery_rank_favoris.c.id_projets_medias
	).outerjoin(
		subquery_rank_possession,
		Projets_Medias.id_projets_medias == subquery_rank_possession.c.id_projets_medias
	).outerjoin(
		subquery_rank_consultations,
		Projets_Medias.id_projets_medias == subquery_rank_consultations.c.id_projets_medias
	).outerjoin(
		subquery_noms_alternatifs,
		Projets_Medias.id_projets_medias == subquery_noms_alternatifs.c.id_projets_medias
	)
	...
	for sub in subquery_popularite:
		produit = produit.outerjoin(
			sub, Projets_Medias.id_fiches == sub.c.id_fiches
		)
	...
	produit = produit.filter(
		Projets_Medias.id_projets_medias == id_projet_media,
		Projets_Medias.verifie == True
	).first()

	return produit

def projet_media_app(session, id_projet_media, client, user_agent):
	info_user = None
	isadulte = False
	# check if projet media exist
	if not session.query(Projets_Medias).filter(Projets_Medias.id_projets_medias == id_projet_media).first():
		return jsonify({"error": "Le Projet Media exigé n'est pas présent dans nos données"}), 404
	...
	if current_user.is_authenticated or get_jwt_identity() is not None:
		...
		sub_query_media_possedes = session.query(
			Posseder_M.id_projets_medias,
			func.count(distinct(case((Posseder_C.pseudo == user, Posseder_C.id_produits_culturels)))).label('produits_culturels_possedes')
		...
		sub_query_produits_culturels = session.query(
			Etre_Compose.id_projets_medias,
			array_agg(aggregate_order_by(Etre_Compose.id_produits_culturels, Etre_Compose.ordre.desc())).label(
				'produits_culturels_ids'),
			array_agg(
				aggregate_order_by(func.coalesce(Posseder_C.pseudo == user, False), Etre_Compose.ordre.desc())).label(
				'produits_culturels_bool_possede')
		).join(Posseder_C, Posseder_C.id_produits_culturels == Etre_Compose.id_produits_culturels, isouter=True) \
			.filter(Etre_Compose.id_projets_medias == id_projet_media) \
		...
		info_user = session.query(
			...
			Posseder_M.date_ajout,
			sub_query_media_possedes.c.produits_culturels_possedes,
			sub_query_produits_culturels.c.produits_culturels_ids,
			sub_query_produits_culturels.c.produits_culturels_bool_possede
		).select_from(Projets_Medias) \
			.outerjoin(Avis, and_(Projets_Medias.id_fiches == Avis.id_fiches, Avis.pseudo == user)) \
			.outerjoin(Notes, and_(Projets_Medias.id_fiches == Notes.id_fiches, Notes.pseudo == user)) \
			.outerjoin(Fiches, Projets_Medias.id_fiches == Fiches.id_fiches) \
			.outerjoin(Etre_Compose, Projets_Medias.id_projets_medias == Etre_Compose.id_produits_culturels) \
			.outerjoin(Posseder_M, and_(Projets_Medias.id_projets_medias == Posseder_M.id_projets_medias,
										Posseder_M.pseudo == user)) \
			.outerjoin(sub_query_media_possedes,
					   Posseder_M.id_projets_medias == sub_query_media_possedes.c.id_projets_medias) \
			.outerjoin(sub_query_produits_culturels,
					   Projets_Medias.id_projets_medias == sub_query_produits_culturels.c.id_projets_medias) \
			.filter(Projets_Medias.id_projets_medias == id_projet_media).first()

	else:
		user = g.user
		isadulte = False

	media = get_objective_data(id_projet_media, session)

	fiche = session.query(Fiches) \
		.select_from(Projets_Medias).join(Fiches) \
	...
	.select_from(Projets_Medias).outerjoin(Etre_Commente_M, Projets_Medias.id_projets_medias == Etre_Commente_M.id_projets_medias) \
	.outerjoin(Commentaires, Etre_Commente_M.id_commentaires == Commentaires.id_commentaires) \
	.filter(Projets_Medias.id_projets_medias == id_projet_media)\
	.filter(Commentaires.signale == False)\
	.order_by(Commentaires.date_post.desc()).all()

	media_is_adulte = session.execute(select(Fiches.adulte).join(Fiches).select_from(Projets_Medias).filter(Projets_Medias.id_fiches == Fiches.id_fiches).filter(Projets_Medias.id_projets_medias == id_projet_media))
	...
	reponse = {
		...
		"media": {column: getattr(media, column) for column in media._fields},
	...
	if isadulte or media_is_adulte or session.get('adulte', False):
		if client == 'app':
		...
		if client == 'app':
			...
			return render_template("projet_media.html", activate_adulte_js_verification=True, **reponse)
